...
def test_list_ratings_returns_user_scoped_ratings():
    ...
    service = RatingService(repository=repository)
...
def test_create_rating_adds_user_scoped_payload():
    ...
    rating = service.create_rating(
        repository.user_id,
        RatingCreate(platform_id=1, rating=1250),
    )

    assert rating["user_id"] == str(repository.user_id)
    assert rating["platform_id"] == 1
...
def test_create_rating_raises_404_for_missing_game():
    ...
    with pytest.raises(HTTPException) as exc:
        service.create_rating(
            repository.user_id,
            RatingCreate(platform_id=1, rating=1250, game_id=uuid4()),
        )
...
def test_create_rating_accepts_own_game():
    ...
    rating = service.create_rating(
        repository.user_id,
        RatingCreate(platform_id=1, rating=1250, game_id=repository.game_id),
    )
