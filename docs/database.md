...
|profile_platform_ratings|プラットフォーム別現在レート|
...
profiles ||--o{ profile_platform_ratings : owns

platforms ||--o{ profile_platform_ratings : rated
```
...
# 16. profile_platform_ratings

## 概要

ユーザーごとのプラットフォーム別「現在のレート/段級位」を保持します。

`ratings`(対局ごとの履歴)とは独立しており、ユーザーがプロフィール画面から手動で登録・更新します。

未プレイのプラットフォームは `has_played = false` とし、その場合 `rating`/`rank` は保存しません(常に NULL)。
...
|------|---|----|----|
|user_id|UUID|NO|ユーザー|
|platform_id|SMALLINT|NO|プラットフォーム|
|has_played|BOOLEAN|NO|プレイ済みかどうか(デフォルト false)|
|rating|INTEGER|YES|現在のレート(レート制プラットフォーム)|
|rank|TEXT|YES|現在の段級位(段位制プラットフォーム)|
|updated_at|TIMESTAMPTZ|NO|更新日時|
...
|profiles|profile_platform_ratings|1:N|
|platforms|games|1:N|
|platforms|ratings|1:N|
|platforms|profile_platform_ratings|1:N|
|openings|games|1:N|
...
|profile_platform_ratings|物理削除(CASCADE)|

ゲームを削除した場合は、関連する `game_tags` は自動的に削除されます。プロフィールを削除した場合は、関連する `profile_platform_ratings` も自動的に削除されます。
...
## 20.8 favorite_openings

本人のみ操作できます。

---

## 20.9 profile_platform_ratings
