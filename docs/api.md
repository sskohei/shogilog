...
全エンドポイントが `X-Request-ID` レスポンスヘッダーを返す(issue QA-3で追加)。リクエストに同ヘッダーが付いていればその値をそのまま返し、無ければサーバー側で発行する。サーバー側のAccess Log/Error Logの `request_id` と一致するため、問い合わせ時の相関に利用できる。詳細は `docs/backend.md` §10 を参照。
